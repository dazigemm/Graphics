...
function gl_start(canvas, vertexShader) {           // START WEBGL RUNNING IN A CANVAS
   ...
   setTimeout(function() {
	 ...
	 if (shaderError.length == 0)
	    ...
	    var lineNumber = parseInt(message) - 2;
	    message = 'error at line ' + lineNumber + message.substring(message.indexOf(' '), message.length);
...
function setIndex(i) {
   ...
   highlight.setHighlight(fss[index]);
...
function addTextEditor(fss, callback) {                      // Add a text editor to the web page:
   ...
   highlight.setHighlight = function(hlight) {
      var str = '';
      for (var i = 0 ; i < hlight.length ; i++)
         str += (hlight.charCodeAt(i) > 32 ? '&block;&block;' : '  ') + '\n';
      this.innerHTML = '<hr size=' + (isChrome() ? 10 : 15) + ' color=black><pre>' + str + '</pre>';
   }

   highlight.setHighlight(hlight);
